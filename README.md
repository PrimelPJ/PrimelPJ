<h1 align="center">Hi 👋 I'm Primel Jayawardana</h1>

---

## 🪐 About Me:

I’m a University of Calgary student interested in software engineering, fintech, and building clean, useful products. I enjoy learning by building, whether that’s apps, APIs, or research-heavy projects.

---

## 🚀 What I'm Up To

- 📱 Building iOS apps with **Swift** and **SwiftUI**
- 🤖 Exploring Machine Learning with **scikit-learn**
- 🗄️ Building backend apps with **Spring Boot** and **PostgreSQL**
- 🔎 Researching web scanning tools (cookies, trackers, and beacons)

Check out my **coding portfolio**: [primelj.dev](https://primelj.dev)

---

## ⭐ Featured Project

### 🏋️ Gym — Personal Gym Tracker (iOS)
A clean, minimal workout tracker built with **SwiftUI** focused on fast logging and simple progress stats.

- ✅ 3-tab layout: Home • Workout • Profile
- 🔥 Streak + weekly activity preview
- 📊 Daily summary (Sessions, Sets, Volume) + profile stats
- 🌙 Dark mode support

**Repo:** https://github.com/PrimelPJ/Gym

---

## 🧠 About Me

Check out my Instagram!

- 📸 [Instagram](https://instagram.com/primel_jayawardana_)

---

## 🌐 Connect with me:

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue)](https://www.linkedin.com/in/primelj)

---

## 💻 Tech Stack:

### Languages:
![Python](https://img.shields.io/badge/Python-111111?style=for-the-badge&logo=python)
![Java](https://img.shields.io/badge/Java-111111?style=for-the-badge&logo=openjdk)
![SQL](https://img.shields.io/badge/SQL-111111?style=for-the-badge&logo=postgresql)
![HTML5](https://img.shields.io/badge/HTML5-111111?style=for-the-badge&logo=html5)
![CSS3](https://img.shields.io/badge/CSS3-111111?style=for-the-badge&logo=css3)
![JavaScript](https://img.shields.io/badge/JavaScript-111111?style=for-the-badge&logo=javascript)
![Swift](https://img.shields.io/badge/Swift-111111?style=for-the-badge&logo=swift)

### Developer Tools:
![Xcode](https://img.shields.io/badge/Xcode-111111?style=for-the-badge&logo=xcode)
![Postman](https://img.shields.io/badge/Postman-111111?style=for-the-badge&logo=postman)
![PowerBI](https://img.shields.io/badge/Power%20BI-111111?style=for-the-badge&logo=powerbi)
![Git](https://img.shields.io/badge/Git-111111?style=for-the-badge&logo=git)

### Libraries/Frameworks:
![SwiftUI](https://img.shields.io/badge/SwiftUI-111111?style=for-the-badge&logo=swift)
![AWS](https://img.shields.io/badge/AWS-111111?style=for-the-badge&logo=amazonaws)
![React](https://img.shields.io/badge/React-111111?style=for-the-badge&logo=react)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-111111?style=for-the-badge&logo=postgresql)
![Spring Boot](https://img.shields.io/badge/Spring%20Boot-111111?style=for-the-badge&logo=springboot)
![Node.js](https://img.shields.io/badge/Node.js-111111?style=for-the-badge&logo=node.js)
![Microsoft Azure](https://img.shields.io/badge/Microsoft%20Azure-111111?style=for-the-badge&logo=microsoftazure)
![Docker](https://img.shields.io/badge/Docker-111111?style=for-the-badge&logo=docker)
